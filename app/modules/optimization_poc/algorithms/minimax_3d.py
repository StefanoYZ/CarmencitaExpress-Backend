...
from app.modules.optimization_poc.utils.constants import MINIMAX
...
def order_packages(packages: list[Package3D]) -> list[Package3D]:
    return sorted(packages, key=lambda package: package_sort_key(package, MINIMAX))
...
def _score(candidate: Placement, truck: Truck3D, placed: list[Placement]) -> tuple[float, ...]:
    ...
    worst_waste = max(x_waste, y_waste, z_waste)
